...
/// (--)
Node::Node(std::string _name, int _id, IsoPos *_rel_iso_pos)
{
    name = _name;
    id = _id;
...
/// \param rel_iso_pos The IsoPos of the node with only relative values set
...
// (--)
Node *Node::attachNewNode(std::string _name, int _id, IsoPos *_rel_iso_pos, int weight1, int weight2, int debugLevel)
{
    ...
    if(debugLevel >= 1) {
        std::cout << "curr.rel_iso_pos (" << rel_iso_pos->rely << ", " << rel_iso_pos->relx << ")    newNode.iso_pos ( " << _rel_iso_pos->rely << ", " << _rel_iso_pos->relx << ")\n";
    }
    bool aboveOf = false;
    ...
    if(_rel_iso_pos->rely < rel_iso_pos->rely) {
        aboveOf = true;
    }
    // are we right of current node?
    if(_rel_iso_pos->relx > rel_iso_pos->relx) {
        rightOf = true;
    }
...
/// Simple "attach functions", both directions:
...
IsoPos *Node::get_copy_rel_iso_pos()
{
