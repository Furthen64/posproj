...
HurkaMap *FileManager::readRegularFile(std::string fullUri, int debugLevel, IsoMatrix *isomat)
{
    ...
    if (infile.is_open())
    {
        ...
        int **matrix = allocateMatrix(isomat->getRows(), isomat->getCols());
        ...
        if(debugLevel >= 2) {
            ...
            dumpMatrix(matrix, isomat->getRows(), isomat->getCols(), ind2);
        }

        // Create output object now that we have the matrix
        resultMap = new HurkaMap(fullUri, matrix, isomat->getRows(), isomat->getCols());
       ...
       while(yDown < resultMap->getRows())
       {
           ...
           while(yUp > -1 && xRight < resultMap->getCols())
           {
               ...
               if(matrix[yUp][xRight] != 0) {
                ...
                textureName = res->getTextureNameByIndex( matrix[yUp][xRight] );

                Block *block  = new Block(new IsoPos(yUp, xRight), textureName);
       ...
       while(xDownRight < resultMap->getCols())
       {
           ...
           while(yUp > -1 && xRight < resultMap->getCols())
           {
                ...
                if(matrix[yUp][xRight] != 0) {
                    ...
                    textureName = res->getTextureNameByIndex( matrix[yUp][xRight] );    // 001 -> "HOUSE001" for instance
...
// (-+)
bool FileManager::saveRegularFile(std::string fullUri, int debugLevel, HurkaMap *hmap, IsoMatrix *isomat)
{
