...
void RoadNetwork::dump(std::string indent)
{
    ...
    std::cout << indent << "absolute iso position = " << rect->absStart->y << ", " << rect->absStart->x << "\n";
...
// (--) test
Bus *RoadNetwork::busAtPos(IsoPos *searchPos)
{
...
// (-+)
IsoPos *RoadNetwork::getNrRoad_iso(int findNr, int debugLevel)
{

    IsoPos *iso_pos = new IsoPos(-1,-1);
    ...
    for(int y = 0; y <= YMAX; y++) {
        for(int x = 0; x <= XMAX; x++) {
            ...
            if(hMatrix->matrix[y][x] == 1)
            {
                ...
                iso_pos->y = rect->absStart->y + y;
                iso_pos->x = rect->absStart->x + x;
...
SlotPath *RoadNetwork::createSlotPath(IsoPos *from_rel_iso_pos, IsoPos *to_rel_iso_pos, int debugLevel )
{
    ...
    Graph *graph = new Graph("road_network_1", rect->absEnd->y + 1, rect->absEnd->x + 1);
...
void RoadNetwork::createGraphFromHMatrix(HurkaMatrix *roadMatrix,
                                         ...
                                         Node *currNode,
                                         Node *prevNode,
                                         IsoPos *curr_rel_iso_pos,
                                         IsoPos *prev_rel_iso_pos,
                                         ...
                                         int dbgLevel)
{
    ...
    IsoPos *up_iso = curr_rel_iso_pos->clone();
    up_iso->rely -= 1;

    up_iso->rely = up_iso->rely;
    up_iso->relx = up_iso->relx;



    IsoPos *right_iso = curr_rel_iso_pos->clone();
    right_iso->relx += 1;

    right_iso->rely = right_iso->rely;
    right_iso->relx = right_iso->relx;




    IsoPos *down_iso = curr_rel_iso_pos->clone();
    down_iso->rely += 1;

    down_iso->rely = down_iso->rely;
    down_iso->relx = down_iso->relx;



    IsoPos *left_iso = curr_rel_iso_pos->clone();
    left_iso->relx -= 1;

    left_iso->rely = left_iso->rely;
    left_iso->relx = left_iso->relx;
    ...
    */
    ...
    if(up_iso->rely >= 0) { // Not hit the wall yet?
        if(roadMatrix->matrix[(int)up_iso->rely][(int)up_iso->relx] == 1) { // is it a road?
    ...
    /// Go right
    if(right_iso->relx <= (roadMatrix->cols-1)) {
        if(roadMatrix->matrix[(int)right_iso->rely][(int)right_iso->relx] == 1) {
    ...
    /// Go down
    if(down_iso->rely <= (roadMatrix->rows-1)) {
        if(roadMatrix->matrix[(int)down_iso->rely][(int)down_iso->relx] == 1) {
    ...
    /// Go left
    if(left_iso->relx >= 0) {
        if(roadMatrix->matrix[(int)left_iso->rely][(int)left_iso->relx] == 1) {
...
void RoadNetwork::createSlotPathFromDijkstraResult(DijkstraResult *dijkstraResult, SlotPath *slotpath, int debugLevel)
{
  ...
  //  CanvasPos *work_cpos;
    ...
    while( !stacker->empty())
    {
        ...
        // Get the iso_pos
        IsoPos *temp_isopos = workNode->get_copy_rel_iso_pos();

        // Now adjust to full gamematrix by using roadnetworks offsets
        temp_isopos->y = temp_isopos->rely  + rect->absStart->y;
...
/*
        ...
        workPos->x = Grid::convert_iso_to_x(workPos->y, workPos->x, GRID_TEXTURE_WIDTH, GRID_TEXTURE_HEIGHT, 2);   // rendered as a GRID
        */
